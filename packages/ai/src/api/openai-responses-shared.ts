	...
	for (const msg of transformedMessages) {
		if (msg.role === "user") {
			...
			const textResult = msg.content
				...
				.join("\n");
			const hasImages = msg.content.some((content): content is ImageContent => content.type === "image");
			const [callId] = msg.toolCallId.split("|");
			let output: string | ResponseFunctionCallOutputItemList;
			if (hasImages && model.input.includes("image")) {
				const parts: ResponseFunctionCallOutputItemList = [];
				if (textResult) parts.push({ type: "input_text", text: sanitizeSurrogates(textResult) });
				for (const block of msg.content) {
					if (block.type === "image") {
						parts.push({
							type: "input_image",
				...
				output = sanitizeSurrogates(textResult || (hasImages ? "(see attached image)" : "(no tool output)"));
			}
...
export async function processResponsesStream<TApi extends Api>(
	...
	const getOrCreateSlot = (outputIndex: number, item: ResponseOutputItem): ResponsesOutputSlot | undefined =>
		outputSlots.get(outputIndex) ?? createSlot(outputIndex, item);
	...
	for await (const event of openaiStream) {
		if (event.type === "response.created") {
			...
			stream.push({ type: "toolcall_delta", contentIndex: slot.contentIndex, delta: event.delta, partial: output });
			...
			const delta = event.arguments.startsWith(previousPartialJson)
				? event.arguments.slice(previousPartialJson.length)
				: "";
			if (delta) stream.push({ type: "toolcall_delta", contentIndex: slot.contentIndex, delta, partial: output });
