...
import { createAssistantMessageEventStream } from "@enterprise-agent/ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `agent-harness-${Date.now()}-${Math.random().toString(36).slice(2)}`);
