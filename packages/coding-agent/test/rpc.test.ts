...
import type { AgentEvent } from "@enterprise-agent/agent-core";
 ...
 * RPC mode tests.
 */
describe.skipIf(!process.env.LITELLM_API_KEY)("RPC mode", () => {
	let client: RpcClient;
	let sessionDir: string;

	beforeEach(() => {
		sessionDir = join(tmpdir(), `agent-rpc-test-${Date.now()}`);
		client = new RpcClient({
			cliPath: join(__dirname, "..", "dist", "cli.js"),
			cwd: join(__dirname, ".."),
			env: { EAGENT_HOME: sessionDir },
			provider: "litellm",
	...
	test("should get state", async () => {
		...
		expect(state.model?.provider).toBe("litellm");
		expect(state.model?.id).toBe("enterprise-default");
