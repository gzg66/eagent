...
import { fauxAssistantMessage, fauxToolCall, type Model } from "@enterprise-agent/ai";
...
describe("AgentSession prompt characterization", () => {
	...
	it("expands skill commands before sending the prompt", async () => {
		const tempDir = join(tmpdir(), `agent-skill-${Date.now()}-${Math.random().toString(36).slice(2)}`);
