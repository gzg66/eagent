...
describe("ExtensionRunner", () => {
	...
	describe("provider registration", () => {
		it("bindCore ignores invalid queued registrations and reports extension error", async () => {
			...
			expect(errors).toEqual([
				'/tmp/broken-extension.ts: Provider litellm: "api" is required when registering streamSimple.',
			]);
		...
		it("pre-bind unregister removes all queued registrations for a provider", () => {
			...
			runtime.registerProvider("litellm", providerModelConfig);
			...
			runtime.unregisterProvider("litellm");
		...
		it("post-bind register and unregister take effect immediately", () => {
			...
			runtime.registerProvider("litellm", providerModelConfig);
			expect(runtime.pendingProviderRegistrations).toHaveLength(0);
			expect(modelRegistry.find("litellm", "instant-model")?.cost.tiers).toEqual([
				{
					inputTokensAbove: 272000,
					input: 2,
					output: 3,
			...
			runtime.unregisterProvider("litellm");
			expect(modelRegistry.find("litellm", "instant-model")).toBeUndefined();
		});
	});
