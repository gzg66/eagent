...
function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "agent-format-resume-command-"));
	tempDirs.push(dir);
...
function createSessionManager(options: {
	...
	return {
		...
		getSessionDir: () => options.sessionDir ?? "/tmp/agent-sessions",
...
describe("formatResumeCommand", () => {
	...
	it("includes unquoted safe session dirs for non-default session dirs", () => {
		...
		const sessionManager = createSessionManager({
			...
			sessionDir: "/tmp/custom-agent-sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir /tmp/custom-agent-sessions --session test-session`,
		);
	});
	...
	it("quotes session dirs containing single quotes", () => {
		...
		const sessionManager = createSessionManager({
			sessionFile,
			sessionId: "test-session",
			sessionDir: "/tmp/custom agent's sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir '/tmp/custom agent'\\''s sessions' --session test-session`,
		);
	});
	...
	it("returns undefined when the session file is missing", () => {
		setStdoutIsTTY(true);
		const sessionManager = createSessionManager({ sessionFile: "/tmp/agent-missing-session.jsonl" });
