...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".eagent", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".eagent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".eagent", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".eagent"), { recursive: true });
			writeFileSync(join(cwd, ".eagent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".eagent"), { recursive: true, force: true });
