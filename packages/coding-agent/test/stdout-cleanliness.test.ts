...
function createTempDir(): string {
	const dir = mkdtempSync(join(tmpdir(), "agent-stdout-clean-"));
	tempDirs.push(dir);
	return dir;
}
...
async function runCli(args: string[]): Promise<{ stdout: string; stderr: string; code: number | null }> {
	...
	const projectConfigDir = join(projectDir, ".eagent");
...
describe("stdout cleanliness in non-interactive modes", () => {
	...
	it("keeps stdout empty for --mode json --help without running configured package commands", async () => {
	...
	it("keeps stdout empty for -p --help without running configured package commands", async () => {
