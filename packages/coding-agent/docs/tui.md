> agent can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@enterprise-agent/tui`](../../../packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@enterprise-agent/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `EAGENT_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@enterprise-agent/tui";
...
**In custom tools** via `agent.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = agent.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@enterprise-agent/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@enterprise-agent/tui";
```
...
```typescript
import { matchesKey, Key } from "@enterprise-agent/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@enterprise-agent/tui";
...
```typescript
import { getMarkdownTheme } from "@enterprise-agent/coding-agent";
import { Markdown } from "@enterprise-agent/tui";
...
Set `EAGENT_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
EAGENT_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@enterprise-agent/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@enterprise-agent/coding-agent";
import { DynamicBorder } from "@enterprise-agent/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@enterprise-agent/tui";
...
```typescript
import { BorderedLoader } from "@enterprise-agent/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@enterprise-agent/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@enterprise-agent/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@enterprise-agent/tui";
...
Customize the inline working indicator shown while agent is streaming a response.
...
// Restore agent's default spinner
ctx.ui.setWorkingIndicator();
```
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@enterprise-agent/coding-agent";
import { matchesKey, truncateToWidth } from "@enterprise-agent/tui";
