...
Enterprise Agent saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.eagent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
eagent -c                  # Continue most recent session
eagent -r                  # Browse and select from past sessions
eagent --no-session        # Ephemeral mode; do not save
eagent --name "my task"    # Set session display name at startup
eagent --session <path|id> # Use a specific session file or partial session ID
eagent --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `eagent -r` opens the same picker at startup.
...
When available, agent uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
eagent --name "Refactor auth module"
eagent --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `eagent -r`.
...
When `/tree` switches away from one branch to another, agent can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
