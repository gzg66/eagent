> agent can create themes. Ask it to build one for your setup.
...
Enterprise Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.eagent/themes/*.json`
- Project: `.eagent/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `agent.themes` entries in `package.json`
...
On first run, agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.eagent/themes
vim ~/.eagent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, agent reloads it automatically for immediate visual feedback.
...
Enterprise Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, agent falls back to the nearest approximation.
