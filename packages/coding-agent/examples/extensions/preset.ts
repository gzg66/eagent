 ...
 * - ~/.eagent/presets.json (global)
 * - <cwd>/.eagent/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `eagent --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "@enterprise-agent/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@enterprise-agent/tui";

// Preset configuration
interface Preset {
	/** Provider name; must be "litellm". */
	provider?: string;
	/** Model ID as configured behind LiteLLM. */
	model?: string;
	/** Thinking level */
