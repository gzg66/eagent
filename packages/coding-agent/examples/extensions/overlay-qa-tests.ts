 ...
 *
 * Usage: eagent --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@enterprise-agent/coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@enterprise-agent/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@enterprise-agent/tui";
...
export default function (agent: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like agent-doom would need)
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/enterprise-agent/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/enterprise-agent\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like agent-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (agent-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
