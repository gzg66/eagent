 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit agent cleanly.
 */
...
export default function (agent: ExtensionAPI) {
	// Register a /quit command that cleanly exits agent
	agent.registerCommand("quit", {
		description: "Exit agent cleanly",
	...
	agent.registerTool({
		...
		description: "Complete a task and exit agent",
	...
	agent.registerTool({
		...
		description: "Deploy the application and exit agent",
		parameters: Type.Object({
