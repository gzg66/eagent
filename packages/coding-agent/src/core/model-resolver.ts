...
import { type Api, type KnownProvider, type Model, modelsAreEqual } from "@enterprise-agent/ai";
 ...
 * Handles gateway model IDs that contain colons.
 *
 * Algorithm:
 ...
 * 1. Prefer an alias over a dated model version.
 * 2. If no alias, pick the latest dated version
 *
 * Supports gateway model IDs that contain colons.
...
export async function resolveModelScopeWithDiagnostics(
	...
	for (const pattern of patterns) {
		...
		if (pattern.includes("*") || pattern.includes("?") || pattern.includes("[")) {
			...
			// This allows a bare model pattern to match without a provider prefix.
...
export function resolveCliModel(options: {
	...
	// A known provider prefix takes precedence over a slash inside a model ID.
	...
	// This handles gateway model IDs that naturally contain slashes.
	if (!provider) {
	...
	// A full gateway model ID may contain both slashes and a thinking suffix.
	if (inferredProvider) {
		const lower = cliModel.toLowerCase();
		const exact = availableModels.find(
			(m) => m.id.toLowerCase() === lower || `${m.provider}/${m.id}`.toLowerCase() === lower,
	...
	if (provider) {
		...
		// For example, "team/model:high" separates the model ID and thinking level.
