...
Update installed packages or refresh explicitly configured model catalogs.
...
export async function handlePackageCommand(
	...
	try {
		switch (options.command) {
			...
			case "update": {
				const target = options.updateTarget ?? { type: "extensions" };
				if (target.type === "extensions") {
					await packageManager.update(target.source);
					console.log(chalk.green(target.source ? `Updated ${target.source}` : "Updated packages"));
				}
				return true;
			}
		}
