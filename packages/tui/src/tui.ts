 ...
 */
export const CURSOR_MARKER = "\x1b_agent:c\x07";
...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.EAGENT_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.EAGENT_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.EAGENT_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".eagent", "agent", "eagent-debug.log");
		...
		// Configurable via setClearOnShrink() or EAGENT_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".eagent", "agent", "eagent-crash.log");
				const crashData = [
		...
		if (process.env.EAGENT_TUI_DEBUG === "1") {
