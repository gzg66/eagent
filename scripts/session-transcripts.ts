 ...
 *   --analyze      Spawn agent subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("agent", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn agent: ${err.message}`));
...
async function main() {
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn agent subagents for pattern analysis.");
		return;
	}
