...
const agentDirEnvName = "EAGENT_HOME";
const startupBenchmarkEnvName = "EAGENT_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific EAGENT_HOME for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force EAGENT_OFFLINE=1
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "agent-startup-benchmark-")) : undefined;
