...
const packages = [
	{ directory: "packages/ai", name: "@enterprise-agent/ai" },
	{ directory: "packages/tui", name: "@enterprise-agent/tui" },
	{ directory: "packages/agent", name: "@enterprise-agent/agent-core" },
	{ directory: "packages/coding-agent", name: "@enterprise-agent/coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "agent-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `agent-${platform}.zip` : `agent-${platform}.tar.gz`;
...
function createPiShim(installDirectory) {
	...
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "agent.cmd"))) {
			writeFileSync(join(installDirectory, "agent.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\agent.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "agent.ps1"), '& "$PSScriptRoot/node_modules/.bin/agent.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "agent.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\agent.exe" %*\r\n');
		writeFileSync(join(installDirectory, "agent.ps1"), '& "$PSScriptRoot/node_modules/.bin/agent.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "agent"), join(installDirectory, "agent"));
}
...
if (rootPackageJson.name !== "enterprise-agent-monorepo") {
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `agent-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "agent.exe" : "agent")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "agent.cmd" : "agent")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "agent.cmd" : "agent")} --help`);
	}
}
